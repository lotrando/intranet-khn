        ...
        <div class="card-body p-4">
          ...
          <form action="{{ route('user.bikes.update', $bike->id) }}" method="POST">
            ...
            <div class="row">
              ...
              <div class="col-2 mb-3">
                <label class="form-label" for="user_name">{{ __('Jméno') }}</label>
                <input class="form-control @error('user_name') is-invalid @enderror" id="user_name" name="user_name"
                  type="text" readonly
                  value="{{ old('user_name') }}@isset($bike){{ $bike->user->user_name }}@endisset">
                @error('user_name')
                  <span class="invalid-feedback" role="alert">
                    {{ $message }}
                  </span>
            ...
            <a class="btn btn-secondary" href="{{ route('user.bikes.index') }}">{{ __('Back') }}</a>
